pub fn convert_winit_event(event: winit::event::Event<()>) -> Option<crate::event::Event> {
	...
	match event {
		...
		W::RedrawRequested(window_id) => Some(C::WindowEvent(crate::event::WindowRedrawRequestedEvent {
			window_id,
		}.into())),
...
pub fn convert_winit_device_event(device_id: winit::event::DeviceId, event: winit::event::DeviceEvent) -> crate::event::DeviceEvent {
	...
	use crate::event;
	match event {
		W::Added => event::DeviceAddedEvent {
			device_id,
		}.into(),
		W::Removed => event::DeviceRemovedEvent {
			device_id,
		}.into(),
		...
		W::Motion { axis, value } => event::DeviceMotionEvent {
			device_id,
			axis,
			value,
		}.into(),
		...
		W::Text { codepoint } => event::DeviceTextInputEvent {
			device_id,
			codepoint,
		}.into(),
	}
...
pub fn convert_winit_window_event(window_id: winit::window::WindowId, event: winit::event::WindowEvent) -> Option<crate::event::WindowEvent> {
	use winit::event::WindowEvent as W;
	use crate::event;

	#[allow(deprecated)]
	match event {
		W::Resized(size) => Some(event::WindowResizedEvent {
			window_id,
			size,
		}.into()),
		W::Moved(position) => Some(event::WindowMovedEvent {
			window_id,
			position,
		}.into()),
		W::CloseRequested => Some(event::WindowCloseRequestedEvent {
			window_id,
		}.into()),
		W::Destroyed => Some(event::WindowDestroyedEvent {
			window_id,
		}.into()),
		W::DroppedFile(file) => Some(event::WindowDroppedFileEvent {
			window_id,
			file,
		}.into()),
		W::HoveredFile(file) => Some(event::WindowHoveredFileEvent {
			window_id,
			file,
		}.into()),
		W::HoveredFileCancelled => Some(event::WindowHoveredFileCancelledEvent {
			window_id,
		}.into()),
		W::ReceivedCharacter(character) => Some(event::WindowTextInputEvent {
			window_id,
			character,
		}.into()),
		W::Focused(true) => Some(event::WindowFocusGainedEvent {
			window_id,
		}.into()),
		W::Focused(false) => Some(event::WindowFocusLostEvent {
			window_id,
		}.into()),
		W::KeyboardInput { device_id, input, is_synthetic } => Some(event::WindowKeyboardInputEvent {
			window_id,
			device_id,
			input: convert_winit_keyboard_input(input),
			is_synthetic,
		...
		W::CursorMoved { device_id, position, modifiers } => Some(event::WindowCursorMovedEvent {
			window_id,
			device_id,
			position,
			modifiers,
		}.into()),
		W::CursorEntered { device_id } => Some(event::WindowCursorEnteredEvent {
			window_id,
			device_id,
		}.into()),
		W::CursorLeft { device_id } => Some(event::WindowCursorLeftEvent {
			window_id,
			device_id,
		}.into()),
		W::MouseWheel { device_id, delta, phase, modifiers } => Some(event::WindowMouseWheelEvent {
			window_id,
			device_id,
			delta,
			phase,
		...
		W::MouseInput { device_id, state, button, modifiers } => Some(event::WindowMouseInputEvent {
			window_id,
			device_id,
			button,
			state: convert_winit_element_state(state),
		...
		W::TouchpadPressure { device_id, pressure, stage } => Some(event::WindowTouchpadPressureEvent {
			window_id,
			device_id,
			pressure,
			stage,
		}.into()),
		W::AxisMotion { device_id, axis, value } => Some(event::WindowAxisMotionEvent {
			window_id,
			device_id,
			axis,
			value,
		}.into()),
		W::Touch(touch) => Some(event::WindowTouchEvent {
			window_id,
			touch,
		}.into()),
		W::ThemeChanged(theme) => Some(event::WindowThemeChangedEvent {
			window_id,
			theme: convert_winit_theme(theme),
		}.into()),
		W::ScaleFactorChanged { scale_factor, .. } => Some(event::WindowScaleFactorChangedEvent {
			window_id,
			scale_factor,
		}.into()),
	}
